...
class _ScanQRState extends State<ScanQR> {
  ...
  Future _validateStudent() async{
    ...
    final response = await http
        .post('https://hacktoberfest.abhijithvijayan.in/api/v1/getStudentDetails', body: body, headers: {"Content-Type": "application/json"});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      :Column(
        children: <Widget>[
          (student == null)?Padding(
            padding: const EdgeInsets.only(top:100.0),
            child: Image.asset('assets/qr.png'),
          ):Text(student.status.toString()),
          Padding(
            padding: const EdgeInsets.only(top:20.0),
            child: RaisedButton(child: Text("Scan"),
